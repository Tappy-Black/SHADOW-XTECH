...
cmd({  
  ...
  if (!isOwner) {  
    return reply("⚠️ This command is only for the bot owner.");  
  }  
  ...
  try {  
    ...
    const { data: commitData } = await axios.get("https://api.github.com/repos/Tappy-Black/SHADOW-XTECH/commits/main");  
    ...
    if (latestCommitHash === currentHash) {  
      return reply("```✅ Your SHADOW-XTECH bot is already up-to-date!```\n");  
    }  

    await reply("```SHADOW-XTCEH Bot Updating...🚀```\n");  
      
    // Download latest code  
    const zipPath = path.join(__dirname, "latest.zip");  
    const { data: zipData } = await axios.get("https://github.com/Tappy-Black/SHADOW-XTECH/archive/main.zip", { responseType: "arraybuffer" });  
