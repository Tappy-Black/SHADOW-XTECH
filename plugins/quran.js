...
cmd({
  ...
  try {
    ...
    if (!surahInput) {
      return reply('☪️ Type Surah Number or Type *.Surahmenu* for getting Surah numbers');
    }
    ...
    if (!surahData) {
      return reply(`☪️ Couldn't find surah with number or name "${surahInput}"`);
    }
    ...
    if (!res.ok) {
      ...
      return reply(`☪️ API request failed with status ${res.status} and message ${error.message}`);
    }
